...
function paintBust(ctx, f, opts) {
  ...
  if (!opts.companion) {
    ...
    const aTop = kid ? 99 : 96, aBot = kid ? 120 : 120;
    for (const s of [-1, 1]) {
      ...
      ctx.closePath(); ctx.fill();
      // cava: sombra separando a manga do tronco (lado interno)
      ctx.lineCap = 'round'; ctx.strokeStyle = 'rgba(0,0,0,.42)'; ctx.lineWidth = 1.7;
      ctx.beginPath(); ctx.moveTo(shX - s * wI, aTop + 1); ctx.quadraticCurveTo(shX - s * wI, midY, wrX - s * (wI - 0.6), aBot - 1); ctx.stroke();
      // aresta de luz na frente do braço
      ctx.strokeStyle = 'rgba(240,240,218,.10)'; ctx.lineWidth = 1.1;
      ctx.beginPath(); ctx.moveTo(shX + s * (wO - 1), aTop + 2); ctx.quadraticCurveTo(shX + s * wO, midY, wrX + s * (wI - 1), aBot - 2); ctx.stroke();
      // punho: banda escura na base da manga
      ctx.lineCap = 'butt'; ctx.strokeStyle = 'rgba(0,0,0,.4)'; ctx.lineWidth = (wO + wI) * 0.95;
      ctx.beginPath(); ctx.moveTo(wrX - s * (wI - 0.5), aBot - 4); ctx.lineTo(wrX + s * (wI - 0.5), aBot - 4); ctx.stroke();
      // MÃO saindo do punho (pele — trai a cor do Alternado junto com o rosto)
      const hr = kid ? 2.8 : 3.6;
      ctx.fillStyle = rgb(mix(SK, SH, 0.28));
      ctx.beginPath(); ctx.ellipse(wrX, aBot - 0.5, hr, hr * 1.05, 0, 0, 6.29); ctx.fill();
      ctx.fillStyle = rgb(SK);
      ctx.beginPath(); ctx.ellipse(wrX - s * 0.6, aBot - 1.4, hr * 0.7, hr * 0.7, 0, 0, 6.29); ctx.fill(); // costas da mão na luz
      ctx.strokeStyle = 'rgba(0,0,0,.2)'; ctx.lineWidth = 0.5; // sulcos dos dedos
      for (let k = -1; k <= 1; k++) { ctx.beginPath(); ctx.moveTo(wrX + k * 1.3, aBot + 0.4); ctx.lineTo(wrX + k * 1.3, aBot + hr * 0.9); ctx.stroke(); }
      ctx.restore();
    }
  }
...
function examZoneSVG(f, phys, zone) {
  ...
  if (zone === 'corpo') {
    ...
    const url = renderScene((c) => paintBodyScan(c, f, phys), seed ^ 0xB0D, { w: 150, h: 180, paintScale: 2.7, post: { levels: 8, ditherAmp: 0.2, grain: 3, aberr: 0, scan: 0.06, sat: 0.62 } }).toDataURL();
...
function renderActorFrame(cz, eyesClosed) {
  if (cz && cz.isSilente) return silentePortraitCanvas({ w: 64, h: 77 });
  return renderPortraitCanvas(cz.features, Object.assign({
    w: 64, h: 77, eyesClosed: !!eyesClosed, companion: cz && cz.companion,
    post: { levels: 6, ditherAmp: 0.12, grain: 2, aberr: 0, scan: 0.06, tears: 0, sat: 0.5, contrast: 1.25 },
  }, actorAnomOpts(cz)));
}
