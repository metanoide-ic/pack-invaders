...
(async () => {
  ...
  await dismissModal(); await p.waitForTimeout(600);

  const DAYS = Number(process.env.QA_DAYS || 14);
  let attacksSeen = 0;
  for (let day = 1; day <= DAYS; day++) {
    for (let i = 0; i < 10; i++) { const go = await p.$('#btn-gowork'); if (go && await go.isVisible()) { await go.click(); break; } await dismissModal(); await p.waitForTimeout(300); }
    await p.waitForTimeout(700); await dismissModal(); await p.waitForTimeout(1200);
    for (let c = 0; c < 26; c++) {
      await dismissModal();
      ...
      if (st.threat) {   // atentado: detém antes que ele use o que trouxe
        ...
        await p.waitForTimeout(900); await dismissModal(); await p.waitForTimeout(900);
        continue;
      }
    ...
    await p.waitForTimeout(900); await dismissModal();
