...
function sfx(kind) {
  ...
  try {
    ...
    else if (kind === 'click') { // tique curto de botão/instrumento
      o.type = 'square'; o.frequency.setValueAtTime(2100, t); o.frequency.exponentialRampToValueAtTime(820, t + .02);
      g.gain.setValueAtTime(.045, t); g.gain.exponentialRampToValueAtTime(.001, t + .04); o.start(t); o.stop(t + .05);
    }
...
/* ---------- MÚSICA: "Marcha do Posto Nº 7" (procedural, mas COMPOSTA) ----------
   CORTEJO, não marcha: dobre grave de sino nos tempos fortes, lamento de
   notas compridas em Ré menor com a napolitana (Eb) arrepiando a cada volta,
   percussão surda de préstito e um sino de igreja longe. Sequenciador por
   compasso com lookahead — ritmo cravado, nada de notas soltas ao vento. */
const MUSIC = { on: true, playing: false, timer: null };
const MT = {
  bpm: 64, root: 73.4162,                       // Ré 2 — tônica do cortejo (Ré menor/frígio)
  // 8 compassos: Dm Dm Bb A | Dm Eb(napolitana) A Dm — o Eb é o arrepio
  chords: [[0, 3, 7], [0, 3, 7], [-4, 0, 3], [-5, -1, 2], [0, 3, 7], [1, 5, 8], [-5, -1, 2], [0, 3, 7]],
  // sino grave: só nos tempos 1 e 3 — dobre fúnebre, nada de ostinato saltitante
  bass: [0, null, null, null, 7, null, null, null],
  // frases de 2 compassos (16 colcheias; null = pausa): notas LONGAS, contorno
  // caindo, muito silêncio — um lamento, não um refrão
  mel: [
    [12, null, null, null, 15, null, null, null, 14, null, null, null, 12, null, null, null],
    [15, null, null, null, 17, null, null, 15, 14, null, null, null, null, null, null, null],
    [12, null, null, null, 13, null, null, null, 12, null, null, null, 10, null, null, null],
    [8, null, null, null, 7, null, null, null, 10, null, null, null, null, null, null, null],
  ],
};
const mHz = (s) => MT.root * Math.pow(2, s / 12);
// humor por regime: quanto pior o Estado, mais lenta, fechada e desafinada
const M_MOOD = {
  republica: { det: 3, lp: 1100, tempo: 1 },
  mehrvolk:  { det: 5, lp: 980,  tempo: .96 },
  conselho:  { det: 8, lp: 860,  tempo: .9 },
  colapso:   { det: 14, lp: 700, tempo: .8 },
};
function mMood() { try { return M_MOOD[regimeOfDay(S.day)] || M_MOOD.republica; } catch (e) { return M_MOOD.republica; } }
function mv(fr, t, dur, type, vol, lpf, det, atk) {   // uma voz: osc → lowpass → envelope
  ...
  g.gain.setValueAtTime(.0001, t); g.gain.linearRampToValueAtTime(vol, t + (atk || .015));
...
function mDrum(kind, t, vol) {
  ...
  const n = AC.createBuffer(1, kind === 'snare' ? 5200 : 2400, AC.sampleRate), d = n.getChannelData(0);
  for (let i = 0; i < d.length; i++) d[i] = (Math.random() * 2 - 1) * (1 - i / d.length);
  const s = AC.createBufferSource(); s.buffer = n;
  const f = AC.createBiquadFilter();
  f.type = kind === 'hat' ? 'highpass' : 'bandpass';
  // 'tick' é o toque de relógio abafado; a caixa é surda, de cortejo
  f.frequency.value = kind === 'hat' ? 7000 : kind === 'tick' ? 900 : 1200; f.Q.value = kind === 'tick' ? 2.4 : .9;
  const g = AC.createGain(); g.gain.setValueAtTime(vol || (kind === 'hat' ? .022 : kind === 'tick' ? .04 : .07), t);
  g.gain.exponentialRampToValueAtTime(.001, t + (kind === 'hat' ? .03 : kind === 'tick' ? .05 : .12));
  s.connect(f); f.connect(g); g.connect(AC.destination); s.start(t); s.stop(t + .14);
}
function musicTick() {  // agenda um compasso à frente (lookahead de ~0.6s)
  ...
  try {
    ...
    const late = (bar % 32) >= 16;                       // segunda metade: entra o harmônico frio
    for (let i = 0; i < 8; i++) {
      const t = t0 + i * e8;
      // DOBRE grave — sino de bronze nos tempos 1 e 3, deixado soar
      const b = MT.bass[i];
      if (b != null) {
        mv(mHz(chord[0] + b - 12), t, e8 * 3.6, 'sine', .075, 300, 0);
        mv(mHz(chord[0] + b - 12), t, e8 * 3.6, 'sawtooth', .02, 260, mood.det);
      }
      // percussão de cortejo: bumbo surdo no 1, o tique de relógio no 3,
      // e um rufo abafado de caixa fechando cada 4º compasso
      if (i === 0) mDrum('kick', t, .17);
      if (i === 4) mDrum('tick', t, .038);
      if ((bar % 4) === 3 && i >= 6) { mDrum('snare', t, .028); mDrum('snare', t + e8 * .5, .018); }
      // LAMENTO — voz dupla desafinada, notas compridas que se apagam
      const semi = phrase[off + i];
      if (semi != null) {
        mv(mHz(semi), t, e8 * 3.4, 'triangle', .038, mood.lp, 0);
        mv(mHz(semi), t, e8 * 3.4, 'sawtooth', .013, mood.lp * .8, mood.det + 4);
        // o harmônico frio: uma quinta acima, quase inaudível, chega tarde
        if (late) mv(mHz(semi + 7), t + e8 * .5, e8 * 2.6, 'sine', .011, 1400, -mood.det, .3);
      }
    }
    // no compasso do Eb (napolitana), um CLUSTER respira por baixo: ré e mi
    // bemol juntos, ataque lento — a dissonância bate devagar, como mau agouro
    if ((bar % 8) === 5) {
      mv(mHz(0), t0, e8 * 8, 'sine', .02, 380, 0, e8 * 3);
      mv(mHz(1), t0, e8 * 8, 'sine', .017, 380, mood.det, e8 * 3);
    }
    // sino de igreja distante a cada 16 compassos (parciais inarmônicas)
    if (bar % 16 === 0) {
      [[1, .015], [2.76, .007], [5.4, .003]].forEach(([h, a]) => mv(146.83 * h, t0, 4.2, 'sine', a, 2600, 0, .004));
    }
    MUSIC.next = t0 + 8 * e8;
    MUSIC.bar = bar + 1;
  } catch (e) {}
...
function startMusic() {
  ...
  try {
    ...
    MUSIC.drone = [];
    // Ré 1 + Ré 2 + um Fá 2 quase inaudível: a terça menor mora no zumbido
    [[36.7, 1], [73.42, .4], [87.31, .16]].forEach(([fr, a]) => {
...
function drawTitleCrest(lidAmt) {
  ...
  if (lidAmt > 0) {
    x.save();
    x.beginPath(); x.moveTo(C - 46 * u, ey); x.quadraticCurveTo(C, ey - 34 * u, C + 46 * u, ey); x.quadraticCurveTo(C, ey + 34 * u, C - 46 * u, ey); x.closePath(); x.clip();
    const lidY = ey - 34 * u + lidAmt * 68 * u;
    x.fillStyle = '#e6dcc0'; x.fillRect(C - 48 * u, ey - 34 * u, 96 * u, lidAmt * 68 * u);   // a pálpebra
    x.fillStyle = 'rgba(120,96,58,.22)'; x.fillRect(C - 48 * u, lidY - 3 * u, 96 * u, 3 * u); // dobra
    x.strokeStyle = GOLD; x.lineWidth = 1.6 * u; x.beginPath(); x.moveTo(C - 46 * u, lidY); x.lineTo(C + 46 * u, lidY); x.stroke(); // linha de cílios
    x.restore();
  }
...
function startTitleSnow() {
  drawTitleCrest();
  TS.blinkT = null; TS.nextBlink = null;
  const cv = $('title-snow');
  if (!cv) return;
  ...
  const loop = () => {
    ...
    TS.t += .016;
    // PISCAR do olho do brasão (ocasional, rápido) — redesenha o brasão só na piscada
    if (TS.nextBlink == null) TS.nextBlink = TS.t + 3 + Math.random() * 4;
    if (TS.blinkT != null) {
      TS.blinkT += .016; const ph = TS.blinkT / .32;
      if (ph >= 1) { drawTitleCrest(0); TS.blinkT = null; TS.nextBlink = TS.t + 3.5 + Math.random() * 5; }
      else drawTitleCrest(ph < .5 ? ph * 2 : (1 - ph) * 2);
    } else if (TS.t >= TS.nextBlink) { TS.blinkT = 0; }
...
function startAmbience() {
  ...
  try {
    ...
    // zumbido elétrico fraco (100 Hz, harmônico da rede) — a lâmpada/válvulas do posto
    const oh = AC.createOscillator(); oh.type = 'triangle'; oh.frequency.value = 100;
    const gh = AC.createGain(); gh.gain.value = .0035; oh.connect(gh); gh.connect(AC.destination); oh.start();
    // "ar da sala": ruído filtrado em loop, bem baixo (ventilação) — tira o vazio digital
    ...
    const nf = AC.createBiquadFilter(); nf.type = 'lowpass'; nf.frequency.value = 500; nf.Q.value = 0.4;
    const ng = AC.createGain(); ng.gain.value = .006;
    ns.connect(nf); nf.connect(ng); ng.connect(AC.destination); ns.start();
    droneNodes = { o1, o2, oh, ns, g };
    // eventos diegéticos esparsos: alguém tosse na fila, uma cadeira arrasta,
    // um aviso abafado no alto-falante. Vêm "de outra sala" (grave e filtrado)
    // — o posto passa a soar habitado, não um loop vazio.
...
function scheduleAmbienceEvent() {
  ...
  ambTimer = setTimeout(() => { ambienceEvent(); scheduleAmbienceEvent(); }, 11000 + Math.random() * 20000);
}
function ambienceEvent() {
  if (!SFX_ON || !droneNodes) return;
  try {
    if (!AC || AC.state === 'suspended') return;
    const t = AC.currentTime;
    // tudo passa por um lowpass duro: soa "atrás da parede"
    ...
    if (kind === 0) { // tosse: dois pulsos de ruído com formante
      for (let k = 0; k < (Math.random() < .5 ? 1 : 2); k++) {
        const tt = t + k * 0.22, s = noise(0.16);
        const bp = AC.createBiquadFilter(); bp.type = 'bandpass'; bp.frequency.value = 420; bp.Q.value = 1.2;
        const g = AC.createGain(); g.gain.setValueAtTime(0, tt); g.gain.linearRampToValueAtTime(.09, tt + .015); g.gain.exponentialRampToValueAtTime(.0005, tt + .15);
        s.connect(bp); bp.connect(g); g.connect(wall); s.start(tt); s.stop(tt + .16);
      }
    } else if (kind === 1) { // cadeira arrastando: sawtooth grave deslizando + atrito
      const o = AC.createOscillator(); o.type = 'sawtooth'; o.frequency.setValueAtTime(70, t); o.frequency.linearRampToValueAtTime(52, t + .5);
      const g = AC.createGain(); g.gain.setValueAtTime(0, t); g.gain.linearRampToValueAtTime(.05, t + .08); g.gain.linearRampToValueAtTime(0, t + .55);
      const s = noise(0.55), sf = AC.createBiquadFilter(); sf.type = 'bandpass'; sf.frequency.value = 240; const sg = AC.createGain(); sg.gain.value = .03;
      o.connect(g); g.connect(wall); s.connect(sf); sf.connect(sg); sg.connect(wall); o.start(t); o.stop(t + .56); s.start(t); s.stop(t + .55);
    } else if (kind === 2) { // aviso abafado: sino de dois tons (o "din-don" do PA)
      [[660, t], [523, t + .34]].forEach(([fr, tt]) => {
        const o = AC.createOscillator(); o.type = 'sine'; o.frequency.value = fr;
        const g = AC.createGain(); g.gain.setValueAtTime(0, tt); g.gain.linearRampToValueAtTime(.045, tt + .03); g.gain.exponentialRampToValueAtTime(.0006, tt + .5);
        o.connect(g); g.connect(wall); o.start(tt); o.stop(tt + .52);
      });
    } else if (kind === 3) { // passos: dois baques macios
      for (let k = 0; k < 2; k++) {
        const tt = t + k * 0.34, o = AC.createOscillator(); o.type = 'sine'; o.frequency.setValueAtTime(90, tt); o.frequency.exponentialRampToValueAtTime(48, tt + .1);
        const g = AC.createGain(); g.gain.setValueAtTime(.06, tt); g.gain.exponentialRampToValueAtTime(.0005, tt + .13);
        o.connect(g); g.connect(wall); o.start(tt); o.stop(tt + .14);
      }
    } else { // papel farfalhando: rajadas curtas de ruído agudo
      for (let k = 0; k < 3; k++) {
        const tt = t + k * 0.09 + Math.random() * .04, s = noise(0.06);
        const hp = AC.createBiquadFilter(); hp.type = 'highpass'; hp.frequency.value = 1600;
        const g = AC.createGain(); g.gain.setValueAtTime(0, tt); g.gain.linearRampToValueAtTime(.02, tt + .01); g.gain.linearRampToValueAtTime(0, tt + .05);
        s.connect(hp); hp.connect(g); g.connect(out); s.start(tt); s.stop(tt + .06);
      }
    }
  } catch (e) {}
}
function stopAmbience() {
  ...
  if (!droneNodes) return;
  try { droneNodes.o1.stop(); droneNodes.o2.stop(); if (droneNodes.oh) droneNodes.oh.stop(); if (droneNodes.ns) droneNodes.ns.stop(); } catch (e) {}
  droneNodes = null;
}
...
function makeFig(x) {
  const child = chance(.12);   // de vez em quando, uma criança na fila (humaniza a multidão)
  return {
    x, tx: x, phase: rnd() * 6.28, h: child ? 19 + ri(0, 5) : 32 + ri(0, 9), child,
    coat: pick(COAT_COLORS), hat: child ? (chance(.5) ? 2 : 0) : ri(0, 3), skin: pick(SKINS), hair: pick(HAIRC),
    scarf: chance(child ? .7 : .5) ? pick(SCARF_COLORS) : null, wide: child ? .62 + rnd() * .16 : .86 + rnd() * .34,
...
// tique tátil ao acionar ferramentas e perguntas do interrogatório
document.addEventListener('click', (ev) => { if (ev.target.closest && ev.target.closest('.btn-tool, #ask-row button')) sfx('click'); }, true);
$('btn-music').onclick = () => {
  MUSIC.on = !MUSIC.on;
