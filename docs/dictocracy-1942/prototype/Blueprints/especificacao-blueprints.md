...
- Contém: `BP_PlayerCharacter` posicionado no ponto de spawn; `BP_NPC_Ismay`, `BP_NPC_Brooke`, `BP_NPC_Eden` posicionados na "sala de mapas"; um `BP_DocumentProp` (mesa com documento); um `BP_StrategicMapTerminal` (objeto interativo que abre o widget de mapa); um `BP_DamageStateManager` (Actor vazio que escuta o indicador `GermanIndustrialOutput` do `DecisionSystemSubsystem` e troca o material de uma parede/mapa de parede quando o valor cruza um limiar — esta é a "pequena alteração visual condicionada a estado de jogo" exigida).
